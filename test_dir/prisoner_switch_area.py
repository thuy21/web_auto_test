...
def prisoner_switch_area(page):
    # 生成上账模板
    js_ver = 'window.scrollTo(0,1000);'
    list = xlsx_read('./tmp_file/import.xlsx', '转监区')
    xls_update('./tmp_file/批量转大队模板.xls', list, 1)

    page.driver.find_element_by_link_text('基础信息管理').click()
    page.driver.find_element_by_link_text('戒毒人员转大队管理').click()
    ...
    page.input_file_upload.send_keys(os.path.abspath('./') + '/tmp_file/批量转大队模板.xls')
    sleep(2)
    page.prisoner_switch_area_upload_file.click()
    sleep(2)
